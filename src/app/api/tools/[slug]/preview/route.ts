...
import { SESSION_ID_COOKIE } from "@/lib/analytics/events";
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const result = await createPreview({ slug, intakeRaw: body.intake, ip, sessionId: store.get(SESSION_ID_COOKIE)?.value ?? null });
