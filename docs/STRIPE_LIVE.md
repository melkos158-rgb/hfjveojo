...
Status 2026-09-26 16:52 UTC: **LIVE.** Steps 1–5 done — live key (in `STRIPE_SECRET_KEY`), destination `we_1UJyViGsFrMfnr38ljrQ8d6L`, signing secret, live probe verified 16:42, `STRIPE_MODE=live` set by the owner; `/admin/system` reads "Customer checkouts: LIVE", account `acct_1UIuDZGsFrMfnr38 · orvionis · ORVIONIS` (public name changed from "jarvis" by the owner). Open: Stripe payouts not enabled yet (money is collected; payouts start once Stripe finishes its checks — Dashboard notifications / Settings → Payouts). Step 6 (opening a live Checkout page without paying) is blocked for the operator by the safety classifier as a real-world transaction — the owner's first real order (step 7) covers it. Admin sandbox testing is off until the sandbox key is back in `STRIPE_SECRET_KEY` and the live key moves to `STRIPE_LIVE_SECRET_KEY`.
