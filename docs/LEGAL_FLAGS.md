...
| 8 | **Cookie consent / Google Analytics** | Essential cookies (session, attribution, anonymous id) plus, when `NEXT_PUBLIC_GA_MEASUREMENT_ID` is set, GA4 with Consent Mode v2: ads storage always denied; analytics storage denied by default in the EEA/UK/CH until "Accept analytics", granted by default elsewhere with an "Essential only" opt-out (advanced consent mode sends cookieless pings before consent). Stricter reading of ePrivacy would load GA only after consent everywhere — decide with a professional; switch by changing the default in `src/lib/ga.ts`. Privacy page names Google as a processor. | `CookieConsent.tsx`, `src/lib/ga.ts`, `/privacy` |
