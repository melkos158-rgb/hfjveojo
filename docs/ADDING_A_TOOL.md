...
- Priced per unit (several photos in one order): a `rooms` intake field (`type: "rooms"`, `max`, room types in `options`; the form sends a JSON array of `{ photoFileId, roomType }` — normalise it in the schema with `z.preprocess`), `quantity: (intake) => units`, `photoInputs: (intake) => [{ fileId, label }]` (claims the uploads, feeds the before/after grid and the original-photo page), `pricing.unit: { one: "photo", many: "photos" }` and `landing.ctaLabelMany: "Stage {n} photos — {total}"`. The server charges `unit price × quantity` (Stripe line item quantity) and stores `Order.quantity`; the per-order AI cap scales with it. Tag image outputs with `content.room` so the order page groups them per room.
